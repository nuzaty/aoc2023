...
function calculateRemainingBad(
    group: number,
    badAmount: number,
    badAmountRecord: number[],
) {
    let totalBad = 0;
...
function addBadSpringData(
    nextPermutaions: SpringPermutationData[],
    currData: SpringPermutationData,
    badAmountRecord: number[],
) {
    ...
    if (
        calculateRemainingBad(group, badAmount, badAmountRecord) >
        currData.remainingSpring + 1
    ) {
        return;
    }
...
function newGoodSpringData(
    ...
    // case next group
    if (badAmount > 0) {
        if (badAmount === badAmountRecord[group]) {
            // check case invalid next group (the amount must be match with record data)
            const newGroup = group + 1;
            const existing = nextPermutaions.find(
                el =>
                    el.permutationData.group === newGroup &&
        ...
        const existing = nextPermutaions.find(
            el =>
                el.permutationData.group === group &&
                el.permutationData.badAmount === 0,
        );
...
export default async function (isPart1: boolean): Promise<number> {
    ...
    for await (const line of readLines('./src/day12/input.txt')) {
        ...
        const springRecord = isPart1
            ? token[0]
            : Array(5).fill(token[0]).join('?');
        const badAmountRecord: number[] = spiltWith(
            ',',
            isPart1 ? token[1] : Array(5).fill(token[1]).join(','),
        ).map(el => Number(el));
        ...
        for (let i = 0; i < springRecord.length; i++) {
            ...
            if (i === 0) {
                ...
                if (currSpring === '?') {
                    nextPermutaions.push({
                        ...initData,
                        permutationData: {...permutationData, badAmount: 1},
                        currSpring: '#',
                    });
        ...
        const sumSelectedPermutaions = currentPermutaions
            .filter(
                ({permutationData}) =>
                    (permutationData.group === matchGroup1 &&
                        permutationData.badAmount === matchAmount1) ||
                    (permutationData.group === matchGroup2 &&
                        permutationData.badAmount === matchAmount2),
            )
